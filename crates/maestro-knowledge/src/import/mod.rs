...
//! ([`ImportCompleted`](maestro_kernel::journal::ImportCompleted)), on its
//! collection's stream.
//!
...
//! other. A new revision is recorded with its hold in one write, so no crash
//! leaves one of them undecided; one an earlier import recorded, before its
//! `source_ref` was shared, is held when the import finds it shared. Either
//! counts as held. The quality gate leaves a revision that has a disposition
//! alone.
//!
