...
//! stored as two artifacts and recorded as a revision, then held when its
//! manifest gives its `source_ref` other bytes too.
...
/// `source_ref` other bytes, which holds its revision back. A revision the
/// kernel records already is left as it is, and nothing is written for it.
pub(super) fn import(
    target: &Target<'_>,
    corpus: &impl Corpus,
    entry: &Entry,
    ...
    let recorded = match target
        .database
        .revision(target.scopes, &canonical.revision_id)?
    {
        Some(_) => Recorded::Unchanged,
        None => record(target, entry, &document_id, &markdown, &canonical)?,
    };
    if !shared {
        return Ok(match recorded {
            Recorded::New => Imported::New,
            Recorded::Unchanged => Imported::Unchanged,
        });
    }
    match hold(target, &canonical.revision_id, &entry.source_ref)? {
        Recorded::New => Ok(Imported::Held),
        Recorded::Unchanged => Ok(Imported::Unchanged),
    }
}
...
/// as artifacts, then records its document and its revision, which pins
/// both.
fn record(
    target: &Target<'_>,
    entry: &Entry,
    ...
    let canonical_digest = database.put(&saved(canonical)?, "application/json")?;
    ...
    Ok(database.record_revision(&revision)?)
}

/// `document` as maestro-canonicalization saves it, pretty JSON and a final
/// newline, so that the artifact holds the bytes `save_document` writes.
fn saved(document: &CanonicalDocument) -> Result<Vec<u8>, Reason> {
    let mut json =
...
/// Holds the revision `revision_id` of `target`'s source back: quarantined
/// by the import, since lines of its manifest give `source_ref` different
/// digests, and neither may replace the other until someone decides.
fn hold(
    target: &Target<'_>,
    revision_id: &str,
    source_ref: &str,
) -> Result<Recorded, document::Error> {
    target.database.record_disposition(&Disposition {
        revision_id: revision_id.to_owned(),
        outcome: Outcome::Quarantined,
        reasons: vec![format!(
            "lines of the manifest of the source {} give the source_ref {source_ref} different \
             digests: neither replaces the other until someone decides",
            target.source
        )],
