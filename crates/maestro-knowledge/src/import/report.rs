...
pub struct Report {
    ...
    /// The entries recorded as new revisions.
    pub imported: u64,
    /// The entries whose revision was recorded before, as it is.
    pub unchanged: u64,
    /// The entries whose revision this import held back, because another
    /// line of their manifest gives their `source_ref` other bytes.
    pub held: u64,
...
pub(super) enum Imported {
    /// A new revision.
    New,
    /// Nothing: its revision, and its hold if it has one, were recorded
    /// before.
    Unchanged,
    /// Its revision's hold, and the revision itself if it is new.
    Held,
}
