...
impl Scratch {
    ...
    /// A connection to the kernel's database file of its own, as a program
    /// outside the kernel opens it: the database must exist already.
    pub(super) fn outside(&self) -> Connection {
        let file = self.0.join("kernel").join("kernel.sqlite3");
        Connection::open_with_flags(file, OpenFlags::SQLITE_OPEN_READ_WRITE).unwrap()
    }
}
...
pub(super) fn counts(report: &Report) -> [u64; 4] {
    [
        report.imported,
        report.unchanged,
        report.held,
