...
Artifact paths are hashes of document ID, revision ID and serialized artifact, so caller IDs cannot become path traversal. Different parser options, operational metadata or asset observations can produce distinct artifacts for one source revision. Storage targets a local filesystem with hard links on Linux, macOS or Windows, each tested in CI. On Linux and macOS the pinned `rustix` dependency supplies safe directory-relative operations; on Windows the standard library's Win32 open flags do: each path component and artifact is opened without following symlinks, junctions or other reparse points. Captured directory handles prevent ancestor replacement from redirecting reads/writes; on Windows every component is held open against renaming and removal. Nonregular files are refused; FIFO opens cannot hang the loader. Directory entries are synced with `fsync` on Linux and macOS; Windows flushes a directory only through a writable handle and skips it, so a power loss there can drop the newest entry, which the next save recreates ([ADR-0018](../../docs/adr/0018-rustix-on-unix-and-win32-flags-on-windows.md)). The output directory the caller names is trusted: its links, such as macOS's `/tmp` into `/private/tmp`, resolve once, so one path behaves the same on every platform; below it, every link is refused.
