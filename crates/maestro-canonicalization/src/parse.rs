...
fn tag_kind(tag: Tag<'_>) -> Kind {
    let block = match tag {
        ...
        Tag::Heading {
            ...
            level: level as u8,
            explicit_id: id.map(|text| text.to_string()),
            classes: classes.iter().map(ToString::to_string).collect(),
            attributes: attrs
                .into_iter()
                .map(|(key, value)| (key.to_string(), value.map(|text| text.to_string())))
                .collect(),
        },
        ...
        Tag::BlockQuote(kind) => BlockAttributes::BlockQuote {
            alert: kind.map(|alert_kind| format!("{alert_kind:?}").to_lowercase()),
        },
