...
impl Mapper<'_> {
            ...
            BlockAttributes::ListItem => {
                ...
                let ordinal = parent
                    ...
                    .filter_map(|child| match child {
                        ContentNode::Block { block_id } => {
                            self.blocks.get(block_id.as_str()).copied()
                        }
                ...
                let marker = match start {
                    Some(start) => format!(
                        "{}. ",
                        u128::from(start)
                            + u128::try_from(ordinal).map_err(|_| invalid_mapping())?
                    ),
                    None => "- ".into(),
                };
    ...
    fn append(
        ...
        let mode = if origins.len() == 1
            && self
                .markdown
                .get(origins[0].span.start..origins[0].span.end)
                == Some(text)
        {
