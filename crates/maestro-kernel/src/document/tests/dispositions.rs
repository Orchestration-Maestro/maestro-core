...
//! they hold a revision back; a revision and its disposition recorded in one
//! write, both or neither.

use super::support::{Scratch, pins, revision};
use crate::{
...
#[test]
fn a_revision_and_its_disposition_are_recorded_in_one_write() {
    let scratch = Scratch::new();
    let database = scratch.open();
    let given = revision(&database, "rev-a", RevisionStatus::Valid);
    let hold = disposition("rev-a", Outcome::Quarantined);
    let recorded = database.record_revision_with_disposition(&given, &hold);
    assert_eq!(recorded.unwrap(), Recorded::New);
    let scopes = ScopeSet::default_workspace();
    assert_eq!(
        database.revision(&scopes, "rev-a").unwrap(),
        Some(given.clone())
    );
    assert_eq!(read(&database, "rev-a"), Some(hold.clone()));
    assert_eq!(pins(&database, &given.original_digest), 1);
    assert_eq!(pins(&database, &given.canonical_digest), 1);
    let again = database.record_revision_with_disposition(&given, &hold);
    assert_eq!(again.unwrap(), Recorded::Unchanged);
    assert_eq!(pins(&database, &given.original_digest), 1, "pinned once");
    let journaled: Vec<Value> = events(&database)
        .into_iter()
        .map(|event| event.data)
        .collect();
    assert_eq!(journaled, [held("rev-a", "quarantined")]);
}

#[test]
fn a_revision_recorded_before_gets_its_disposition_and_other_content_is_refused() {
    let scratch = Scratch::new();
    let database = with_revisions(&scratch, &["rev-a"]);
    let recorded = database.revision(&ScopeSet::default_workspace(), "rev-a");
    let given = recorded.unwrap().unwrap();
    let hold = disposition("rev-a", Outcome::Excluded);
    let decided = database.record_revision_with_disposition(&given, &hold);
    assert_eq!(decided.unwrap(), Recorded::New);
    assert_eq!(read(&database, "rev-a"), Some(hold));
    assert_eq!(pins(&database, &given.original_digest), 1);
    let other = Revision {
        captured_at: Some("2026-09-26T00:00:00Z".to_owned()),
        ..revision(&database, "rev-b", RevisionStatus::Valid)
    };
    database.record_revision(&other).unwrap();
    let rewritten = Revision {
        captured_at: None,
        ..other
    };
    let refused = database
        .record_revision_with_disposition(&rewritten, &disposition("rev-b", Outcome::Quarantined));
    assert!(matches!(refused, Err(Error::RevisionConflict(id)) if id == "rev-b"));
    assert_eq!(read(&database, "rev-b"), None);
    assert_eq!(
        events(&database).len(),
        1,
        "the refused hold journals nothing"
    );
}

#[test]
fn a_revision_whose_disposition_is_refused_is_not_recorded_either() {
    let scratch = Scratch::new();
    let database = scratch.open();
    database
        .write(|transaction| {
    ...
    let given = revision(&database, "rev-a", RevisionStatus::Valid);
    let hold = disposition("rev-a", Outcome::Quarantined);
    let refused = database.record_revision_with_disposition(&given, &hold);
    assert!(matches!(refused, Err(Error::Store(_))), "{refused:?}");
    let scopes = ScopeSet::default_workspace();
    assert_eq!(
        database.revision(&scopes, "rev-a").unwrap(),
        None,
        "no revision is left undecided"
    );
