...
pub(super) const COLUMNS: &str = "id, kind, idempotency_key, attempt, scope, state, \
...
pub struct Job {
    ...
    /// What work it is.
    pub kind: String,
    /// The digest of its kind and its inputs, which a retried command gives
    /// again.
...
impl Database {
    ...
    /// queued, running or succeeded, as it is, or else a new job, queued,
    /// whose attempt follows the last of its key.
    ///
    /// The key is the SHA-256 digest of the JSON array `[kind, inputs]`, in
    /// which the fields of each object follow the order of their names, so
    /// the same inputs make the same key however the caller built them. The
    /// key alone decides: a retried command finds its job whatever scope it
    /// gives.
    ///
    /// # Errors
    ///
    /// [`Error::Store`] when the database cannot record the job, or holds a
    /// job of its key it cannot read back.
    pub fn submit_job(&self, new: &NewJob<'_>, now: SystemTime) -> Result<Job, Error> {
        let key = idempotency_key(new.kind, new.inputs);
        self.write(|transaction| {
            let live = transaction
                .query_row(
                    &format!(
                        "SELECT {COLUMNS} FROM jobs
                         WHERE idempotency_key = ?1 AND state IN ('queued', 'running', 'succeeded')"
                    ),
                    [key.as_str()],
                    job_row,
                )
            ...
            let job = transaction.query_row(
                &format!(
                    "INSERT INTO jobs (id, kind, idempotency_key, attempt, scope, state)
                     SELECT ?1, ?2, ?3, coalesce(max(attempt), 0) + 1, ?4, 'queued'
...
/// The idempotency key of a job of `kind` on `inputs`: `serde_json` writes
/// the fields of an object in the order of their names.
fn idempotency_key(kind: &str, inputs: &Value) -> Digest {
    let named = Value::Array(vec![Value::from(kind), inputs.clone()]);
...
pub(super) fn job_row(row: &Row<'_>) -> rusqlite::Result<Job> {
    ...
    let number = unsigned(row, 6)?;
    let holder: Option<String> = row.get(7)?;
    let heartbeat: Option<String> = row.get(8)?;
    ...
    Ok(Job {
        ...
        state: JobState::named(&state)
            .ok_or_else(|| unreadable(5, format!("no job state is named {state:?}")))?,
        lease: holder
            .zip(heartbeat)
            .zip(expires)
        ...
        outcome: outcome
            ...
            .transpose()
            .map_err(|invalid| unreadable(10, invalid))?,
    })
}
